...
def process_vsix_file(vsix_path):
    ...
    # Create temporary files for handling gzip
    ...
    try:
        ...
        # Extract the decompressed ZIP file
        ...
        try:
            ...
            logging.error(f"The file {extension_name} is not a valid ZIP file after decompression")
            failed_updates += 1
            with open(FAILED_LOG, "a") as f:
                f.write(f"{extension_name}: Not a valid ZIP file after decompression\n")
            return False
        ...
        # Recompress the updated ZIP as gzip
        ...
        shutil.copy2(temp_recompressed_file, vsix_path)
...
def main():
    ...
    # Get all VSIX files
    vsix_files = sorted([os.path.join(EXTENSIONS_DIR, f) for f in os.listdir(EXTENSIONS_DIR) if f.endswith('.vsix')])
    total_extensions = len(vsix_files)
    
    if total_extensions == 0:
        logging.error(f"No VSIX files found in {EXTENSIONS_DIR}")
        sys.exit(1)
    
    logging.info(f"Found {total_extensions} VSIX files to process")
